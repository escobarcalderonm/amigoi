...
{
    public static function replyTweet($idTweet, $user, $text)
    {
        $settings = array(
    ...
    public static function getLastTweet($idUser)
    {
        $settings = array(
            'oauth_access_token' => env("oauth_access_token"),
            'oauth_access_token_secret' => env("oauth_access_token_secret"),
        ...
        $requestMethod = "GET";
        $getfield = '?user_id=' . $idUser . '&count=1';
        $twitter = new TwitterAPIExchange($settings);
    ...
    public static function checkReply()
    {
        $tuiteros = Tuitero::all();
        foreach ($tuiteros as $tuitero) {
            echo '<br>check '.$tuitero->username;
            $tweet = self::getLastTweet($tuitero->id_tuiter);
            $idTweet = $tweet->id;

            $bdtweet = Tuit::where('id_tuit',$idTweet)->first();
            if($bdtweet == null){
                echo '<br>tuit encontrado '.$idTweet;
                $saveTweet = new Tuit();
                $saveTweet->id_tuit = $idTweet;
                $saveTweet->id_tuitero = $tuitero->id;
                $saveTweet->save();
                ...
                $max = 10000000;
                $numero = rand($min, $max);
                self::replyTweet($idTweet, $username, 'Estoy creciendo en Youtube y cuando tenga ' . number_format($numero) . ' subs haré un vídeo contigo. Recordad esto.');
                echo '<br>respondido ';
            }
        }
    }
